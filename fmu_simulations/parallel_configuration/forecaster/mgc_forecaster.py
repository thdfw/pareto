...
try:
    ...
    root = os.getcwd()
    sys.path.append(os.path.join(root, '..', 'forecaster'))
...
from fcWrapper import DEFAULT_CONFIG as default_wrapper
from fcSelector import DEFAULT_CONFIG as default_selector
...
class forecaster_scada(eFMU):
    def __init__(self):
        self.input = {'input-data': None, 'timeout': None, 'fc-map': None, 'tz': None,
                      'hours': None, 'freq': None, 'wf-horizon': None}
...
#         forecast.index = [ix_start+pd.DateOffset(hours=ix) for ix in forecast.index]
        ...
        self.output['wf-data'] = wf.iloc[:self.input['wf-horizon']].to_json() 
...
def default_parameter_forecaster(targetName='irn0_power-pv', backupPath=None, inputPath=None,
                                 ...
                                 trainingIntervalBest=24*60*60, trainingIntervalHyper=14*24*60*60,
                                 min_score=0, trainingHour=0, 
                                 scadaColumnList=scadaColumnList, useExternalTrainer=False,
                                 isExternalTrainer=False, externalTrainerPath=None):
    cfg = {'inputPath': inputPath, # Initialize with database/csv. './forecaster_example_data_simple.csv',
           ...
           'trainingIntervalHyper': trainingIntervalHyper,
           ...
           'useExternalTrainer': useExternalTrainer,
           'isExternalTrainer': isExternalTrainer,
           'externalTrainerPath': externalTrainerPath,
           'min_samples': (min_days+2) * 24*(60*60)/stepsize, # add 2 days buffer; scale by stepsize
           
           # additional config (not used)
           'dataX': None,
           'dataY': None,
           'newRecord': {},
           'csvMetaData': {},
    ...
    # check if keys match
    for k in default_wrapper:
        if not k in cfg:
            print(f'WARNING: Missing key "{k}" in configuration.')

    for k in cfg:
        if not k in default_wrapper:
            print(f'WARNING: Additional key "{k}" in configuration.')
            
    for k in default_selector:
        if not k in cfg['fcParams']:
            print(f'WARNING: Missing key "{k}" in fcParams configuration.')
            
    for k in cfg['fcParams']:
        if not k in default_selector:
            print(f'WARNING: Additional key "{k}" in fcParams configuration.')    
