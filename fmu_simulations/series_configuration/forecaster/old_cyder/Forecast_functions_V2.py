...
if __name__ == '__main__':
    ...
    get_ipython().system('jupyter nbconvert --to script PV_forecast_function_notebook.ipynb')
    get_ipython().system('jupyter nbconvert --to script Forecast_functions_V2.ipynb')
...
def get_forecast_noaa(now, config, timeout=5, wf_prev=pd.DataFrame()):
    dt_fmt = '%Y-%m-%d %H:%M:%S'
    try:
        response = get('http://forecast.weather.gov/MapClick.php?lat='+                        str(config['location_latitude'])+'&lon='+                        str(config['location_longitude'])+'&FcstType=digitalDWML',                        verify=False, timeout=timeout).text.encode('ascii','ignore')
...
def calculate_clearsky(wf, config):
    ...
    wf = wf.join(irradiance.get_total_irradiance(0, 180, wf['apparent_zenith'], wf['azimuth'],                                         wf['dni'],wf['ghi'],wf['dhi']))
    wf = wf.round(1)
    return wf
...
class Forecasting(object):
    ...
    def do_forecast(self,                     obsDf,                     wfDf,
                    predHor=96, \
                    retrainFlag=False):
        # Scale obsDf
        ...
        self.forecastNN = getPVforecast(obsDf, \
                                         ...
                                         resample2CtrlStep)
        '''
        self.forecast5min,         self.forecast15min,         self.forecastSARIMA,         self.forecastNN = getPVforecast_v4(obsDf,                                             wfDf,                                             self.models,                                             predHor,                                             timeStep,                                             timeStepCtrl,                                             tsPeriod,                                             retrainFlag,                                             resample2CtrlStep)
        
        

        predfinal = predHor - 4
        new_ix = [obsDf.index[-1] + pd.DateOffset(minutes=timeStep*(s+1)) for s in range(predfinal)]
        self.forecast = pd.DataFrame({                                       'final':self.forecast15min.reshape(predfinal),                                       'sarima':self.forecastSARIMA.reshape(predfinal),                                       'nn':self.forecastNN.reshape(predfinal)}, index=new_ix)
        # Rescale
...
def example_models1():
    ...
    models['sarima'] = {'path':'models/all/20181031/SARIMA_model_20181025.json',                         'normPowerCoeff': 2626}
    models['nn'] = {'path':'models/all/20181031/NNmodel_best_20181025.sav',                     'normPowerCoeff': 2626, 'normInputData':True,                     'normTa': 30.0, 'normCC': 100.0, 'normCS': 1000.0,                     'architecture':'scalar', #                                temp  cloud sky   Pd-1  hor                    
...
if __name__ == '__main__':
    ...
    config['tz_st'] = -8
    now = (pd.to_datetime(time.time(), unit='s').replace(microsecond=0, nanosecond=0)+           pd.DateOffset(hours=config['tz_st'])).to_pydatetime() # In standard time
...
if __name__ == '__main__':
    ...
    model_paths = dict(sarima='SARIMA_model_20180921.json',                        nn='NNmodel_best_20180921.sav',                        alpha='optimal_weighting_factors_20180921.json')
